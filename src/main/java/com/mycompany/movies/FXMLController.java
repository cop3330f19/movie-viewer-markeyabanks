...
public class FXMLController implements Initializable {
    ...
    private void loadDetailScene(int current){
        ...
        ArrayList<Video> video = movieModel.getVideo(movie.getId());
        ...
        btnBack.setPadding(PAD_10);
        btnBack.setOnAction(new EventHandler<ActionEvent>(){
            @Override
            public void handle(ActionEvent event) {
                //clear the page and reload movie images
               scene.getChildren().clear();
               loadInitialScene();
            }
         });
       ...
       splitPlane.setMinHeight(scene.getHeight());
       splitPlane.setMinWidth(scene.getWidth());
       
       splitPlane.getItems().addAll(lhs, rhs);
       scene.getChildren().add(splitPlane);
